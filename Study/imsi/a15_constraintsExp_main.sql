...
insert into address values(1,1,1,1,1,1);
/*	??? 이건 왜 받아들여지는지 이해가안가네..?  varchar라서 그릉가..? */
/*
	제약조건 추가, 삭제, 수정연습.
...
/*
	제약조건 활성및 비활성화처리
	: 제약조건을 임시적으로 비활성화함으로 데이터를  test하거나,
	특정한 기간에 제약조건이 맞지않는 내용을 입력처리.
	
	형식:
		alter table 테이블명
		enable/disable constraint 제약조건명;
			ex) 테이블  emp07에 emp07_enmae_nn을 비활성화처리.
		
		alter table emp07
		disable constraint emp07_ename_nn;
		select * from user_constraints
		where table_name = 'EMP07';  --- status 컬럼부분확인.
		alter table emp07
		enable constraint emp07_ename_nn; -- 활성화처리.
